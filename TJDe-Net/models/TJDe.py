...
class TJDENET(nn.Module):
    def __init__(self, in_chans=3, out_chans=4, window_size=8,
        ...
        #self.skip1 = nn.Conv2d(embed_dims[0], embed_dims[0], 1)
        self.skip1_improve = nn.Sequential(
            BasicConv(embed_dims[0] + embed_dims[1], embed_dims[0], 3, 1),
            cubic_attention(embed_dims[0], 2, 5)
        )
        # self.skip2 = nn.Conv2d(embed_dims[1], embed_dims[1], 1)
        self.skip2_improve = nn.Sequential(
            BasicConv(embed_dims[1] + embed_dims[0], embed_dims[1], 3, 1),
            cubic_attention(embed_dims[1], 2, 5)
        )
    ...
    def forward_features(self, x):
        ...
        x = self.fusion1([x, self.skip2_improve(torch.cat((skip2,F.interpolate(skip1,(skip2.shape[2],skip2.shape[3]))),1))]) + x
        x = self.layer4(x)
        x = self.patch_split2(x)

        x = self.fusion2([x, self.skip1_improve(torch.cat((skip1,F.interpolate(skip2,(skip1.shape[2],skip1.shape[3]))),1))]) + x
        x = self.layer5(x)
